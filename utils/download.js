...
import { execCommand } from './execCommad.js';
...
export class DownloadAuto extends EventEmitter {
  ...
  async newDownloadRequest(id, url) {
    ...
    const request = { uuid, id, url, source: ' ' };
    this.downloadList = await readFile('./storage/download.json');
  ...
  async runDownload({ uuid, id, url, source = '' }) {
    try {
      ...
      await this.send({ uuid, id, url, source });
      await fs.unlink(source);
      await this.removeDownloadRequest({ uuid, id, url, source });
    } catch (error) {
      await this.removeDownloadRequest({uuid});
      fs.writeFile(source, '');
      await fs.unlink(source);
  ...
  async removeDownloadRequest({ uuid }) {
    const filtered = this.downloadList.filter(r => r.uuid !== uuid);
    console.log({ filtered });
    await writeFile('./storage/download.json', filtered);
  }

  async send({ uuid, id, url, source }) {
    try {
      await this.bot.telegram.sendDocument(id, { source });
    } catch (error) {
